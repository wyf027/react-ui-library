...
function getColumnLabel(column: TableColumn<Record<string, unknown>>) {
...
export const Table = forwardRef<HTMLTableElement, TableProps<Record<string, unknown>>>(
  function Table(
    ...
    const controlledSortColumn = useMemo(
      () => columns.find((column) => column.sortOrder !== undefined),
      [columns],
    )
    ...
    const processedRows = useMemo(() => {
      ...
      if (effectiveSortKey) {
        ...
        if (column?.sorter) {
          rows.sort((a, b) =>
            effectiveSortOrder === 'asc' ? column.sorter!(a, b) : column.sorter!(b, a),
          )
        }
      }
            ...
            <thead className={tableHeadRow}>
              <tr>
                {visibleColumns.map((column) => {
                  ...
                  const nextSortOrder = sorted && effectiveSortOrder === 'asc' ? 'desc' : 'asc'
                  const ariaSort =
                    column.sorter && sorted
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {column.sorter ? (
                          <button
                            ...
                            aria-label={`Sort ${columnLabel} ${nextSortOrder === 'asc' ? 'ascending' : 'descending'}`}
